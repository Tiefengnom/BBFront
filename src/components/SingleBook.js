...
function SingleBook({port}) {
    ...
    const handleClick = async () => {
        const response = await fetch(`https://sore-visor-dove.cyclic.app/bookbandits/collection/${id}`, {
            method: "POST",
            body: JSON.stringify({ borrowed: !fetchedBook.borrowed, owner: fetchedBook.owner,btime: btime, title: fetchedBook.title, borrower: user._id, borrowerfname : user.first_name, borrowerlname: user.last_name, pending: false, image: fetchedBook.image }),
            headers: {
    ...
    fetchedBook && (
        ...
        <div className="flex flex-wrap md:flex-nowrap p-8"  > 
            ...
            <img src={!fetchedBook.image || fetchedBook.image === "none" ? Book : fetchedBook.image } alt='book cover' className="mb-8 max-w-[150px]"/>
            ...
            !fetchedBook.borrowed ? (
                !btime ? (<>
                    ...
                    <button
                        onClick={(e) => {handleClick();alert("You have requested for Permission. You will be notified, if you are denied.")}}
